 ...
 */
public class Vector3 implements Cloneable, NDimensional<Double> {

    /**
     * The first or {@code x}-coordinate of the vector. Can be accessed by {@link #get(int) } with index {@code 0}.
     */
    public double x = 0;
    /**
     * The second or {@code y}-coordinate of the vector. Can be accessed by {@link #get(int) } with index {@code 1}.
     */
    public double y = 0;
    /**
     * The third or {@code z}-coordinate of the vector. Can be accessed by {@link #get(int) } with index {@code 2}.
     */
    public double z = 0;
    ...
    /**
     * Initializes a 2-dimensional vector, that means the third coordinate remains zero.
     *
    ...
    /**
     * Inverts the direction of the vector. Equals the multiplication with the scalar value -1.
     */
    public void invert() {
        x = -x;
        y = -y;
...
//	}
    /**
     * <p>
     * Computes the cross product of this vector and another vector.</p>
     * <p>
     * The product of two vectors {@code a = (a_x, a_y, a_z)} and {@code b = (b_x, b_y, b_z)} is defined as
    ...
    /**
     * <p>
     * Computes the dot, inner or scalar product of this vector and another vector.</p>
     * <p>
     * The scalar product of two vectors {@code a = (a_x, a_y, a_z)} and {@code b = (b_x, b_y, b_z)} is defined as
     * {@code a * b = (a_xb_y + a_yb_y + a_zb_z)}. Note that the result is a scalary value.
     * </p>
     * <p>
     * The multiplication with a scalary value, which returns a vector again, is defined in
     * {@link #scalarMultiplicateTo(double)} and {@link #scalarMultiplicate(double) </p>.}
     *
    ...
    /**
     * Returns a copy of this vector whose coordinates are multiplicated by a scalar value.
     *
    ...
    /**
     * Returns a {@link String}-representation of the vector that is of the type (x; y; z). The numbers in the
     * representation are formatted in the current locale.
     *
    ...
    /**
     * Returns a {@link String}-representation of the vector that is of the type (x; y; z). The numbers in the
     * representation are formatted in the submitted number format.
     *
    ...
    /**
     * Reads a vector from a given string. The string must have the same format as it is printed using the
     * {@link #toString()} method. Note that this depends from the {@link java.util.Locale} scheme that is selected. The
     * string may be enclosed by brackets and/or spaces, also the numbers itself may be surrounded by spaces.
     *
    ...
    /**
     * Reads a vector from a given string. The string must have the same format as it is printed using the
     * {@link #toString()} method. The format of the numbers has to fit to the given number format.
     *
     * @param value the vector as string
    ...
    /**
     * Computes a normal to a plane defined by three vectors or points. It is computed using the formula
     * {@code n = (x - y) \cross (y - z)}, where \cross denotes the
     * {@link #crossProduct(de.tu_berlin.math.coga.math.vectormath.Vector3)}.
     *
    ...
    /**
     * Computes the orientation between two vectors. The third coordinate of the vectors is ignored.
     *
    ...
    public Double get(int index) {
        switch (index) {
            ...
            case 2:
                return z;
            default:
                throw new IllegalArgumentException("Invalid 3d coordinate: " + index);
        }
    }
}
